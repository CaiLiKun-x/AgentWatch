...
**为 Claude Code 和 AI Agent 工作流打造的 Apple Watch 提醒工具。**

AgentWatch 让你不用一直盯着 Claude Code。只有当 Agent 需要授权、需要你处理或任务完成时，它才通过 Apple Watch 震动和简短卡片提醒你。从此告别"离开 20 分钟回来发现 Agent 卡在权限弹窗等了 19 分钟"的尴尬。
...
- 🍎 **Apple Watch 震动 + 短卡片通知** — 标题、风险等级、建议动作一目了然
...
AgentWatch 通过 [Bark](https://apps.apple.com/app/bark/id1403753865)（免费开源 iOS 推送 App）发送通知。
...
你应该在 iPhone 和 Apple Watch 上收到"AgentWatch Bark 测试"通知。

### 确认 Apple Watch 同步
- iPhone：设置 → 通知 → Bark → 允许通知 ✅
- Apple Watch：Watch App → 通知 → 从 iPhone 镜像 Bark 的提醒 ✅
...
### Apple Watch 没有震动
1. 确认 iPhone 上 Bark 安装并正常工作（在 Bark App 内发一条测试消息）
2. 运行 `agentwatch config test` — 应显示"Notification sent"
3. iPhone：设置 → 通知 → Bark → 允许通知 ✅
4. Apple Watch：Watch App → 通知 → 从 iPhone 镜像 Bark ✅
